...
@Component
class FootballMatchesPage : ResponderInterface {
    @Autowired
    lateinit var dsl: DSLContext

    override val customId = "football/menu/page/:page/:timestamp"
    ...
    override suspend fun execute(event: ButtonInteractionEvent) {
        ...
        val timestamp = params.getAsLong("timestamp")!!

        val zoneBr = ZoneId.of("America/Sao_Paulo")
        val referenceInBr = Instant.ofEpochSecond(timestamp).atZone(zoneBr)
        ...
        val menu = footballMatchesMenu(matchesFormatted, event.user.effectiveAvatarUrl, referenceInBr, page)
